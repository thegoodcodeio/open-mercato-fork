...
export default function MyTimesheetsPage() {
  ...
  const goToPrev = React.useCallback(() => {
    if (viewMode === 'weekly') {
      ...
      const targetMonthIndex = monthIndex === 0 ? 11 : monthIndex - 1
      const targetMonthYear = monthIndex === 0 ? monthYear - 1 : monthYear
      setMonthYear(targetMonthYear)
      setMonthIndex(targetMonthIndex)
      setAnchorDate((prev) =>
        clampDateToMonth(prev, targetMonthYear, targetMonthIndex),
      )
    }
  }, [viewMode, monthIndex, monthYear])
  ...
  const goToNext = React.useCallback(() => {
    if (viewMode === 'weekly') {
      ...
      const targetMonthYear = monthIndex === 11 ? monthYear + 1 : monthYear
      setMonthYear(targetMonthYear)
      setMonthIndex(targetMonthIndex)
      setAnchorDate((prev) =>
        clampDateToMonth(prev, targetMonthYear, targetMonthIndex),
  ...
  const loadData = React.useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const selfRes = await readApiResultWithTimeout<{
        member?: { id: string; displayName: string } | null
      }>('/api/staff/team-members/self', undefined, {
        errorMessage: t('staff.timesheets.my.errors.load', 'Failed to load timesheets.'),
      ...
      const assignmentsRes = await readApiResultWithTimeout<{
        items?: Array<Record<string, unknown>>
      }>('/api/staff/timesheets/my-projects?pageSize=100', undefined, {
        errorMessage: t('staff.timesheets.my.errors.load', 'Failed to load timesheets.'),
        fallback: { items: [] },
      })
      ...
      const [projectsRes, entriesRes] = await Promise.allSettled([
        assignedProjectIds.length > 0
          ? readApiResultWithTimeout<{ items?: Array<Record<string, unknown>> }>(
      ...
      const partialLoadFailures: string[] = []

      if (projectsRes.status === 'fulfilled') {
        const projectItems = Array.isArray(projectsRes.value.items) ? projectsRes.value.items : []
        ...
        partialLoadFailures.push('projects')
      }

      if (entriesRes.status === 'fulfilled') {
        const entryItems = Array.isArray(entriesRes.value.items) ? entriesRes.value.items : []
        ...
        partialLoadFailures.push('entries')
      }

      if (partialLoadFailures.length > 0) {
        setLoadError(
          t('staff.timesheets.my.errors.partialLoad', 'Some timesheet data could not be loaded. Try again.'),
        )
      }

    } catch (error) {
      logger.error('staff.timesheets.my.load', { err: error })
      const message = t('staff.timesheets.my.errors.load', 'Failed to load timesheets.')
  ...
  }, [dirty, entries, hasChanges, invalidCellCount, t, loadData, runMutation, mutationContextId, staffMemberId, retryLastMutation])
  ...
  const handleViewModeChange = React.useCallback((mode: ViewMode) => {
    setViewMode(mode)
    if (mode === 'monthly') {
      setMonthYear(anchorDate.getFullYear())
      setMonthIndex(anchorDate.getMonth())
    } else {
      setWeekStart(getMonday(anchorDate))
    }
  }, [anchorDate])
          ...
          <div className="space-y-4">
            ...
            <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-4">
              {Array.from({ length: 4 }).map((_, idx) => (
                <div
                  key={idx}
                  className="h-[92px] animate-pulse rounded-lg border border-border bg-muted/30"
                />
              ))}
            </div>
              ...
              <table className="w-full table-fixed text-sm">
                <colgroup>
                  ...
                  {visibleDays.map((date) => (
                    <col
                      ...
                      className={viewMode === 'weekly' ? '' : 'w-[36px] min-w-[36px]'}
                    />
                  ))}
                  ...
                  <tr className="border-b bg-muted/50">
                    ...
                    {visibleDays.map((date) => {
                      ...
                      return (
                        <th
                          key={formatDateKey(date)}
                          className={`px-1 py-2 text-center font-medium ${weekend ? 'bg-muted/80 text-muted-foreground' : ''}`}
                        >
                ...
                <tbody>
                  <tr className="border-b">
                    ...
                    {visibleDays.map((date) => (
                      <td key={formatDateKey(date)} className="px-1 py-1">
                        <div className="mx-auto h-7 w-10 animate-pulse rounded border border-border bg-muted/30" />
                      </td>
                    ))}
                    <td className="px-3 py-2">
                      <div className="ml-auto h-4 w-8 animate-pulse rounded bg-muted/40" />
                    </td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr className="border-t bg-muted/50 font-semibold">
                    ...
                    {visibleDays.map((date) => (
                      <td key={formatDateKey(date)} className="py-2 text-center text-xs tabular-nums">
                        -
                      </td>
                    ))}
                    <td className="px-3 py-2 text-right tabular-nums font-semibold">0</td>
                  </tr>
                </tfoot>
              </table>
            </div>
      ...
      <PageBody>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            {viewMode === 'weekly' && (
              <CalendarPicker selectedWeekStart={weekStart} onWeekSelect={handleWeekSelect} />
            )}
          </div>
        ...
        {loadError ? (
          <div className="mb-4">
            <ErrorMessage
              label={loadError}
              action={
                <Button size="sm" type="button" variant="outline" onClick={() => { void loadData() }}>
                              ...
                              inputClassName={`text-center text-xs tabular-nums
                                ${cellError
                                  ...
                                  : isDirty
                                    ...
                                    : cellMinutes > 0 ? 'text-foreground' : 'text-muted-foreground'}
