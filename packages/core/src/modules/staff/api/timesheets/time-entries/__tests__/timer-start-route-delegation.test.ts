...
// `POST .../time-entries/{id}/timer-start` used to write the entry directly
// through the ORM, which never invalidated the opt-in CRUD list cache — the same
// defect class as #2609, confirmed empirically: with a key warmed before the
// start, the warmed read kept reporting the entry as not started. The route now
// delegates to the `staff.timesheets.time_entries.start_timer_existing` command,
// so cache invalidation, audit logging and undo come from the command bus. This
// test pins the route's half of that contract: the delegation itself, the
// mutation-guard wiring `packages/core/AGENTS.md` requires for custom write
// routes, the response body, and the error-status mapping.
...
function successfulExecution() {
  return {
    result: { timeEntryId: ENTRY_ID },
    logEntry: {
      id: 'log-1',
      undoToken: 'undo-token-1',
      commandId: 'staff.timesheets.time_entries.start_timer_existing',
...
describe('timer-start route delegation', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  test('delegates to the start_timer_existing command with the scoped input', async () => {
    ...
    // Distinct from `start_timer`, which creates AND starts a new entry. The
    // `staff.timesheets.time_entries.*` prefix is load-bearing either way: the
    // command bus derives the `staff.timesheet` cache alias from it, which is
    // what makes the invalidation tags line up with the cached list payload.
    expect(commandId).toBe('staff.timesheets.time_entries.start_timer_existing')
  ...
  test.each([
    ...
    [409, 'Timer is already started for this entry.'],
    [409, 'Another timer is already running. Stop it before starting a new one.'],
  ...
  test('maps an unexpected command failure to 400', async () => {
    ...
    expect(body).toEqual({ error: 'Failed to start timer.' })
  })
})
